...
SimpleSwitch::SimpleSwitch(int max_port, bool enable_swap)
  ...
  force_arith_field("intrinsic_metadata", "global_timestamp");
...
SimpleSwitch::receive_(int port_num, const char *buffer, int len) {
  ...
  if (phv->has_field("intrinsic_metadata.global_timestamp")) { 
    phv->get_field("intrinsic_metadata.global_timestamp")
        .set(now_timestamp.get_global_timestamp());
  }
...
{  
	...
	int64_t now_64 = (static_cast <int64_t> (POSIX_nanoseconds) / (1e9)) * (one_64bits<<32);
	this->now = now_64;
}


int64_t timestamp::get_global_timestamp()
{
	return this->now;
}
